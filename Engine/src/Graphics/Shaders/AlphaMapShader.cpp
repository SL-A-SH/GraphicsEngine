...
bool AlphaMapShader::Initialize(ID3D11Device* device, HWND hwnd)
{
    ...
    error = wcscpy_s(vsFilename, 128, L"../Engine/assets/shaders/AlphamapVertexShader.hlsl");
    if (error != 0)
    {
        return false;
    }
    ...
    error = wcscpy_s(psFilename, 128, L"../Engine/assets/shaders/AlphamapPixelShader.hlsl");
    if (error != 0)
    {
        return false;
    }
