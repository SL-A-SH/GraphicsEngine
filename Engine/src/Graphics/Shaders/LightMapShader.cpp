...
bool LightMapShader::Initialize(ID3D11Device* device, HWND hwnd)
{
    ...
    error = wcscpy_s(vsFilename, 128, L"../Engine/assets/shaders/LightmapVertexShader.hlsl");
    if (error != 0)
    {
        return false;
    }
    ...
    error = wcscpy_s(psFilename, 128, L"../Engine/assets/shaders/LightmapPixelShader.hlsl");
    if (error != 0)
    {
        return false;
    }
...
bool LightMapShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
...
bool LightMapShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
    XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture1, ID3D11ShaderResourceView* texture2)
{
    HRESULT result;
