...
bool NormalMapShader::Initialize(ID3D11Device* device, HWND hwnd)
{
    ...
    error = wcscpy_s(vsFilename, 128, L"../Engine/assets/shaders/NormalMapVertexShader.hlsl");
    if (error != 0)
    {
        return false;
    }
    ...
    error = wcscpy_s(psFilename, 128, L"../Engine/assets/shaders/NormalMapPixelShader.hlsl");
    if (error != 0)
    {
        return false;
    }
